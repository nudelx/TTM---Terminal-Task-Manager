...
function createTextEditor({
  ...
  function render() {
    if (!isFocused()) {
      ...
      const out = state.lines.map((line, i) =>
        i === state.row ? renderCursorRow(line) : escapeTags(line),
      )
  ...
  function handleKey(ch, key) {
    ...
    if (transition === 'up') {
      ...
      if (next === null) {
        if (typeof onExitUp === 'function') onExitUp()
        return
      }
      state = next
      ...
      if (next === null) {
        if (typeof onExitDown === 'function') onExitDown()
        return
      }
      state = next
    } else {
