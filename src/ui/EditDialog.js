...
function createEditDialog({ screen, theme, keys }) {
  ...
  blessed.text({
    ...
    style: { fg: 'gray' },
    content:
      '{cyan-fg}[enter]{/} save / next   {cyan-fg}[C-enter]{/} newline   {cyan-fg}[tab]{/} field   {cyan-fg}[esc]{/} cancel',
  })
  ...
  keys.bind(screen, 'save', () => {
    if (isOpen()) close(collect())
  })
  keys.bind(screen, 'cancel', () => {
    if (isOpen()) close(null)
  })
  keys.bind(screen, 'next', () => {
    if (isOpen()) focusRing.next(screen)
  })
  keys.bind(screen, 'prev', () => {
    if (isOpen()) focusRing.prev(screen)
  })
